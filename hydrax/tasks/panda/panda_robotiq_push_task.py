...
class PandaRobotiqPushCubeEnv(PandaRobotiqBaseEnv):
    ...
    def _get_termination(self, data: Union[mjx.Data, mjw.Data]):
        ...
        eef_oob = grasp_pos[2] > WORKSPACE_MAX[2]
        eef_oob |= (grasp_pos[0] > WORKSPACE_MAX[0]) | (
                grasp_pos[0] < WORKSPACE_MIN[0]
        )
        eef_oob |= (grasp_pos[1] > WORKSPACE_MAX[1]) | (
                grasp_pos[1] < WORKSPACE_MIN[1]
        )
        hand_wall_collision = [
    ...
    def _get_reward(
        ...
        grasp_box = reward_util.tolerance(
            jp.linalg.norm(box_side_pos - grasp_pos),
            (0, 0.1),
            margin=1.0,
            sigmoid="linear",
        )
        ...
        grasp_collision_side = jp.linalg.norm(hand_box_normal_side)
        ...
        return {
            ...
            "grasp_box": grasp_box,
            "grasp_collision_side": grasp_collision_side,
    ...
    def _get_single_obs(self, data: Union[mjx.Data, mjw.Data], info: dict[str, Any]) -> jax.Array:
        ...
        grasp_pos_w_noise = grasp_pos + jax.random.uniform(
            key3, minval=0, maxval=self._config.noise_config.noise_scales.eef_pos
        )
        grasp_mat = data.site_xmat[self._grasp_site]
        angle = jax.random.uniform(
            key1,
            minval=0,
        ...
        grasp_mat_w_noise = rand_mat @ grasp_mat
